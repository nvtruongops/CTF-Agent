---
name: ctf-crypto
description: Provides cryptanalysis and mathematical verification techniques for educational CTF challenges. Use when analyzing mathematical weaknesses in encryption schemes, hashing, digital signatures, ZKP, PRNG, or solving crypto puzzles involving RSA, AES, ECC, lattices, LWE, CVP, number theory, Coppersmith, Pollard, Wiener, padding oracles, GCM, or key derivation.
license: MIT
...
- **On Windows (WSL Kali Linux)**:
  - Run mathematical scripts and solver tools inside WSL:
    ```bash
    wsl -d kali-linux bash -c "source ~/.ctf-tools/venv/bin/activate && python3 solve.py"
    ```
  - For Hashcat / password cracking:
    ```bash
    wsl -d kali-linux bash -c "hashcat -m <mode> -a 0 hashes.txt wordlist.txt"
    ```
- **On Native Linux / WSL**:
  - Activate the CTF virtualenv:
    ...
    ```
  - Use `hashcat`, `python3` with `z3`, `sympy`, `gmpy2`, and `fpylll` directly.
