---
name: ctf-pwn
description: Provides binary security analysis and proof-of-concept verification for educational CTF challenges. Use when analyzing memory corruption primitives in isolated benchmarks, including buffer overflows, format strings, heap structures, ROP, ret2libc, shellcode analysis, seccomp policy boundaries, sandbox evaluation, or binary test harnesses. Do not use it when the main blocker is understanding binary disassembly; use reverse engineering first. Do not use it for pure web flaws, disk/packet forensics, or standalone crypto problems.
license: MIT
...
- **Manual tools**: `pwndbg` (gdb extension), `checksec` (via pwntools)

## Execution Environment & Tool Invocation

- **On Windows (WSL Kali Linux)**:
  - Run binary analysis and exploit scripts inside WSL:
    ```bash
    wsl -d kali-linux bash -c "source ~/.ctf-tools/venv/bin/activate && python3 solve.py"
    ```
  - For gadget search and one_gadget:
    ```bash
    wsl -d kali-linux bash -c "one_gadget /lib/x86_64-linux-gnu/libc.so.6"
    wsl -d kali-linux bash -c "source ~/.ctf-tools/venv/bin/activate && ROPgadget --binary ./vuln --ropchain"
    wsl -d kali-linux bash -c "source ~/.ctf-tools/venv/bin/activate && ropper --file ./vuln --nocolor"
    ```
- **On Native Linux / WSL**:
  - Activate the CTF virtualenv:
    ...
    ```
  - Tools (`checksec`, `gdb`, `r2`, `one_gadget`, `seccomp-tools`, `ropper`) are directly available in PATH.

## Additional Resources

- [exploit-databases.md](../../references/exploit-databases.md) - Exploit repositories (Exploit-DB, Packet Storm, Sploitus, GitHub PoCs, Google Project Zero, ZDI Advisories) and CLI tool `scripts/cve_lookup.py`.
- [security-events-and-intelligence.md](../../references/security-events-and-intelligence.md) - Pwn2Own exploitation history and techniques, Black Hat / DEF CON archives, OffensiveCon, and automated flag extraction.
- [hard-pwn-lanes.md](hard-pwn-lanes.md) - Hard-Pwn Lane taxonomy & fast exploit selector: 7 canonical lanes (seccomp-orw, sandbox-orw, SROP, ret2dlresolve, heap-fsop House of Apple 2, heap-tcache-stash, shellcode-mmap) with checksec decision tree and pwntools skeletons
