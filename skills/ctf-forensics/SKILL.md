...
- **On Windows (WSL Kali Linux)**:
  - Run forensics carving, memory analysis, and stego tools inside WSL:
    ```bash
    wsl -d kali-linux bash -c "binwalk -Me file.bin"
    wsl -d kali-linux bash -c "exiftool image.jpg"
    wsl -d kali-linux bash -c "tshark -r capture.pcap -Y 'http' -T fields -e http.request.uri"
    wsl -d kali-linux bash -c "zsteg -a image.png"
    wsl -d kali-linux bash -c "source ~/.ctf-tools/venv/bin/activate && vol -f memory.dmp windows.pslist"
    ```
- **On Native Linux / WSL**:
  - Activate the CTF virtualenv:
    ...
    ```
  - Use `vol`, `binwalk`, `tshark`, `zsteg`, `steghide`, and `fls` directly.
